...
These components serve any Lean development, and the course uses them for one specific end. The imperative language of the final lectures, its semantics and its Hoare logic are ordinary Lean definitions, the verification conditions are goals that tactics discharge, and the kernel checks the result as it checks any other proof. The next figure uses these same components and shows what we write with them to verify a program.
